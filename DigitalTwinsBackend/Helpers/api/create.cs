...
namespace DigitalTwinsBackend.Helpers
{
    public partial class Api
    {
        public static async Task<Guid> CreateDeviceAsync(HttpClient httpClient, ILogger logger, Device device)
        {
            ...
            var id = await GetIdFromResponse(response, logger);
            if (id != Guid.Empty)
                await FeedbackHelper.Channel.SendMessageAsync($"device '{device.Name}' successfully created with the ID {id}");
            return id;
        }
        ...
        public static async Task<Guid> CreateEndpointAsync(HttpClient httpClient, ILogger logger, Endpoint endpointCreate)
        {
            ...
            var id = await GetIdFromResponse(response, logger);
            if (id != Guid.Empty)
                await FeedbackHelper.Channel.SendMessageAsync($"endpoint '{endpointCreate.Type}' successfully created with the ID {id}");
            return id;
        }
        ...
        public static async Task<Guid> CreateKeyStoreAsync(HttpClient httpClient, ILogger logger, KeyStore keyStoreCreate)
        {
            ...
            var id = await GetIdFromResponse(response, logger);
            if (id != Guid.Empty)
                await FeedbackHelper.Channel.SendMessageAsync($"keyStore '{keyStoreCreate.Name}' successfully created with the ID {id}");
            return id;
        }
        ...
        public static async Task<Guid> CreateMatcherAsync(HttpClient httpClient, ILogger logger, Matcher matcher)
        {
            ...
            var id = await GetIdFromResponse(response, logger);
            if (id != Guid.Empty)
                await FeedbackHelper.Channel.SendMessageAsync($"matcher '{matcher.Name}' successfully created with the ID {id}");
            return id;
        }
        ...
        public static async Task<Guid> CreateResourceAsync(HttpClient httpClient, ILogger logger, Resource resource)
        {
            ...
            var id = await GetIdFromResponse(response, logger);
            if (id != Guid.Empty)
                await FeedbackHelper.Channel.SendMessageAsync($"resource '{resource.Type}' successfully created with the ID {id}");
            return id;
        }
        ...
        public static async Task<Guid> CreateRoleAssignmentAsync(HttpClient httpClient, ILogger logger, RoleAssignment roleAssignment)
        {
            ...
            var id = await GetIdFromResponse(response, logger);
            if (id != Guid.Empty)
                await FeedbackHelper.Channel.SendMessageAsync($"roleAssignment for RoleId '{roleAssignment.RoleId}' successfully created with the ID {id}");
            return id;
        }
        ...
        public static async Task<Guid> CreateSensorAsync(HttpClient httpClient, ILogger logger, Sensor sensor)
        {
            ...
            var id = await GetIdFromResponse(response, logger);
            if (id != Guid.Empty)
                await FeedbackHelper.Channel.SendMessageAsync($"Sensor '{sensor.HardwareId}' successfully created with the ID {id}");
            return id;
        }
        ...
        public static async Task<Guid> CreateSpaceAsync(HttpClient httpClient, ILogger logger, Space space)
        {
            ...
            var id = await GetIdFromResponse(response, logger);
            if (id != Guid.Empty)
                await FeedbackHelper.Channel.SendMessageAsync($"Space '{space.Name}' successfully created with the ID {id}");
            return id;
        }
        ...
        public static async Task<Guid> CreatePropertyAsync(HttpClient httpClient, ILogger logger, Guid spaceId, PropertyCreate propertyCreate)
        {
            ...
            var id = await GetIdFromResponse(response, logger);
            if (id != Guid.Empty)
                await FeedbackHelper.Channel.SendMessageAsync($"Property '{propertyCreate.Name}' successfully created with the ID {id}");
            return id;
        ...
        public static async Task<Guid> CreatePropertyKeyAsync(HttpClient httpClient, ILogger logger, PropertyKeyCreate propertyKeyCreate)
        {
            ...
            var id = await GetIdFromResponse(response, logger);
            if (id != Guid.Empty)
                await FeedbackHelper.Channel.SendMessageAsync($"PropertyKey '{propertyKeyCreate.Name}' successfully created with the ID {id}");
            return id;
        }
        ...
        public static async Task<Guid> CreateUserDefinedFunctionAsync(HttpClient httpClient, ILogger logger, UserDefinedFunction userDefinedFunction, string js)
        {
            ...
            var id = await GetIdFromResponse(response, logger);
            if (id!= Guid.Empty)
                await FeedbackHelper.Channel.SendMessageAsync($"UserDefinedFunction '{userDefinedFunction.Name}' successfully created with the ID {id}");
            return id;
        }
